local load_now = vim.fn.argc(-1) > 0
local delayed_event = 'VeryLazy'

local now_if_args = function(spec)
  spec.lazy = not load_now
  spec.event = load_now and nil or delayed_event
  return spec
end
...
return {
  {
    ...
    config = function()
      ...
      -- └────────────────────┘
      --
      -- This section contains configuration of the MINI parts of the config.
      -- It contains only configs for the 'mini.nvim' plugin.
      --
      ...
      -- Step one ===================================================================
      -- Color scheme is configured in 'lua/plugins/colorschemes.lua'.
      --
      -- See also:
      -- - `:h mini.nvim-color-schemes` - list of other color schemes
      -- - `:h MiniHues-examples` - how to define highlighting with 'mini.hues'
      -- - 'lua/plugins/colorschemes.lua' honorable mentions - other good color schemes
      ...
      later(function()
        ...
        local add_marks = function()
          ...
          MiniFiles.set_bookmark('p', lazy_plugins, { desc = 'Plugins' })
      ...
      --     - 'friendly-snippets' plugin configured in 'lua/plugins.lua' provides
      ...
      later(function()
        ...
        -- the completion menu. This requires a dedicated in-process LSP server
  ...
  { import = 'plugins.colorschemes' },
  {
    ...
    config = function()
      ...
      -- Use built-in snippet expansion when available (Neovim 0.10+)
      local has_snippet = vim.snippet and vim.snippet.expand

      cmp.setup({
        snippet = {
          expand = function(args)
            -- Only expand snippets if the built-in engine exists
            if has_snippet then
              vim.snippet.expand(args.body)
            end
          end,
        },
        -- Insert mode mappings for completion navigation/confirmation
        ...
        -- Completion sources and their priority order
      ...
      -- Use buffer words for search (`/` and `?`) command line completion
      cmp.setup.cmdline({ '/', '?' }, {
        mapping = cmp.mapping.preset.cmdline(),
        sources = { { name = 'buffer' } },
      })

      -- Use path + command name completion for `:` command line
      cmp.setup.cmdline(':', {
        mapping = cmp.mapping.preset.cmdline(),
