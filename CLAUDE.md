...
- `gh.nix` — GitHub CLI. Owns `~/.config/gh/config.yml` and, via `programs.gh.extensions`, the `~/.local/share/gh/extensions` tree — so extensions come from nixpkgs, never `gh extension install`. `gh skill install` stays imperative (skills aren't packaged)
...
- `tetrio.nix` — TETR.IO desktop client (unfree). Three toggles at the top of the file: `withTetrioPlus` (third-party cosmetic mod — replaces `app.asar`, builds from source, on), `nativeWayland` (sets `NIXOS_OZONE_WL` for this package only, so the Electron client skips Xwayland and with it the fullscreen-origin bug `Mod+g` game mode works around), and `mesaEgl` — **which is load-bearing on `lenovo-yoga`**: glvnd otherwise hands Electron NVIDIA's `libEGL` while sway composites on the Intel iGPU, so no i915 dmabuf can be imported and the client dies in a GL-context-loss loop without ever drawing a frame. Read the comment before touching it. The `tetrio://` scheme handler lives in `xdg.nix` with the other handlers
